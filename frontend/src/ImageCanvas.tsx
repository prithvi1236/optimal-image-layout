...
// ================= SESSION SETUP =================
const getSessionId = () => {
  let sid = localStorage.getItem("layout_studio_sid");
  if (!sid) {
    sid = crypto.randomUUID();
    localStorage.setItem("layout_studio_sid", sid);
  }
  return sid;
};
...
const ImageCanvasStudio: React.FC = () => {
  ...
  const handleDelete = async (idToDelete: string) => {
      ...
      try { await axios.post(`${API_URL}/delete_image`, { image_id: idToDelete }); } catch (err) {}
  };
  ...
  const PageCanvas = ({ pageIndex }: { pageIndex: number }) => {
    ...
    const handleMouseDown = (e: React.MouseEvent) => {
      ...
      if (clickedItem) {
        ...
        setInteraction({ type: handleType ? "resize" : "move", itemId: clickedItem.id, startMouse: { x, y }, initialItem: { ...clickedItem }, handle: handleType || undefined });
    ...
    const selectedItem = pageItems.find(i => i.id === selectedId);

    return (
      <div className="relative w-full h-full">
        <canvas id={`canvas-page-${pageIndex}`} ref={canvasRef} width={A4_WIDTH} height={A4_HEIGHT}
          onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={() => setInteraction(null)} className="block"/>
        {selectedItem && (
            <div className="absolute flex items-center justify-center bg-red-500 text-white rounded-full w-6 h-6 shadow-md cursor-pointer hover:bg-red-600 hover:scale-110 transition-all z-10"
                style={{ left: selectedItem.x + selectedItem.width - 12, top: selectedItem.y - 12 }}
                onClick={(e) => { e.stopPropagation(); handleDelete(selectedItem.id); }} title="Delete Image">
